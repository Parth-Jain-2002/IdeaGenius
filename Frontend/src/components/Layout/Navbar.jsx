...
export default function Navbar(props) {
  ...
  return (
    <section className="flex items-center justify-between mb-4">
      <button onClick={() => { navigate(props.link? props.link: -1) }} className="flex flex-row" title="Go Back">
      ...
      <div className="flex items-center space-x-2 group relative">
        ...
        <div className="group-hover:flex hidden flex-col absolute right-0 top-7 bg-white p-2 text-center rounded-lg shadow-sm">
          ...
          <a href="#" onClick={()=>{logout(); navigate('/')}} className="p-2 text-red-600">Log Out</a>
        </div>
      </div>
    </section>
  )
