...
const EditFormChat = ({ onClose, topics, getTopics, currentTopic, chatId }) => {
  ...
  const handleChangeTopic = () => {
    ...
    axios.post(`http://localhost:8000/edit_topic`, {
    ...
    <div className="fixed top-1/4 left-1/2 transform -translate-x-1/2 bg-white p-8 rounded-md shadow-md w-96 z-50">
      <div className="flex justify-between mb-4">
        <div>
          ...
          <p className="text-gray-500 text-left">Organize your thoughts better</p>
        </div>
      ...
      <div className="flex flex-col space-y-2">
        <label htmlFor="topic" className="text-gray-700 text-left">Choose a topic</label>
        <select
          id="topic"
      ...
      <button onClick={handleChangeTopic} className="bg-gray-300 hover:bg-gray-400 text-white p-2 rounded-full w-full mt-4" disabled={!title}>
          {title}
      </button>
    </div>
    </>
